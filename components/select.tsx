...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-9 w-full items-center justify-between rounded-md border border-gray-200 bg-white px-3 py-2 text-sm shadow-sm ring-offset-gray-200 placeholder:text-gray-900 focus:outline-none hover:ring-2 hover:ring-gray-200 focus:ring-2 focus:ring-gray-200 disabled:cursor-not-allowed disabled:opacity-50 duration-150 transition",
      className ?? ""
    )}
    {...props}
  >
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-pointer select-none items-center rounded-sm py-1.5 pl-2 pr-8 text-sm outline-none focus:bg-gray-200 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className ?? ""
    )}
    {...props}
  >
